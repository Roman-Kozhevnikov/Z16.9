...
private:
	...
	std::map<std::string, std::shared_ptr <User>> _map_us;            // поиск по логину объекта юзера
	std::map<int, std::shared_ptr<Message>> _map_mess;                // поиск по номеру сообщения (обекта сообщения) 
	size_t _mess_count;
